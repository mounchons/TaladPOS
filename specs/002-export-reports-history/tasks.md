...
- [X] T007 [US1] ตรวจสอบด้วยมือตาม quickstart.md หัวข้อ 1 ทั้งหมด (กรณีข้อมูลปกติ, ตารางว่าง 0 แถว
  ต้องได้ไฟล์ที่มีแค่หัวคอลัมน์ตาม FR-011, และยืนยันว่าบัญชี Cashier มองไม่เห็นปุ่มนี้เลยตาม SC-005) —
  ตรวจผ่านเบราว์เซอร์จริงครบทุกเคส (21 แถวตรงกับ API เป๊ะ, ตรวจ .xlsx โดยตรงว่า header ตัวหนา + ภาษาไทยถูกต้อง,
  Cashier เจอ ManagerOnly ทันที)
...
- [X] T010 [US2] ตรวจสอบด้วยมือตาม quickstart.md หัวข้อ 2 ทั้งหมด (จำนวนแถวในไฟล์ตรงกับ pager เกิน 1 หน้าจอ,
  ตัวกรอง "เฉพาะบิลของฉัน" ถูกเคารพ, ตารางว่างได้ไฟล์หัวคอลัมน์อย่างเดียว, เคสเกินเพดานแสดงข้อความแทนที่จะ
  ดาวน์โหลดไฟล์บางส่วน) — ตรวจผ่านเบราว์เซอร์จริง: 77 บิล (4 หน้าจอ) export ได้ครบ 77 แถวในไฟล์เดียว,
  ตัวกรอง "เฉพาะบิลของฉัน" ให้ 18 แถวตรงกับ pager เป๊ะ, ลดเพดานชั่วคราวยืนยัน cap-exceeded message ทำงานถูกต้อง
  (ไม่ดาวน์โหลดไฟล์), Cashier export ได้สำเร็จเช่นกันตาม FR-007
...
- [X] T012 ตรวจสอบด้วยมือตาม quickstart.md หัวข้อ 3 (กันกดปุ่ม export ซ้ำซ้อนระหว่างเตรียมไฟล์ — ทดสอบทั้ง
  สองหน้าจอ, FR-008) — ทำหลัง T007 และ T010 เสร็จแล้วทั้งคู่ — **พบ race condition จริงระหว่างตรวจสอบ**:
  คลิกสองครั้งติดกันเร็วมากหลุดผ่าน guard เดิมที่เช็คจาก React state ได้ (state ยังไม่ update ทันเวลา) แก้เป็น
  เช็คจาก `useRef` แทนใน `ExportButton.tsx` แล้วตรวจซ้ำจนยืนยันว่าเหลือแค่ 1 request ต่อการคลิกสองครั้งติดกัน
