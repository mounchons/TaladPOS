...
>
> **path ในเอกสารนี้เป็นของเดิมโดยตั้งใจ**: รอบนี้ยิงไปที่ `/api/<resource>` ซึ่งเป็น path ที่ใช้อยู่ ณ เวลานั้นจริง
> ภายหลัง T081 ย้ายทุก endpoint ไปเป็น `/api/v1/<resource>` ตาม constitution — บันทึกผลการรันไม่ถูกแก้ย้อนหลัง
> เพราะเป็นหลักฐานว่ารันอะไรไปจริง ไม่ใช่เอกสารอ้างอิง path ปัจจุบัน (ดู contracts/*.md สำหรับ path ที่ใช้ตอนนี้)
