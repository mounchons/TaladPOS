...
|---|---|---|
| `GET /api/v1/products` | **ใช่** | หน้าจัดการสต็อกเป็น datagrid ที่ต้องเลื่อนดูทีละหน้า — **ไม่ใช่**เพราะแคตตาล็อกโตไม่จำกัด (ระดับร้อยถึงพันรายการ ดู Scale/Scope) ความต่างนี้คือเหตุผลที่หน้าขายแสดงสินค้าครบทุกชิ้นได้ตาม FR-037 |
...
**Rationale**: แบ่งหน้าเฉพาะที่**ผู้ใช้ต้องเลื่อนดูทีละหน้าจริง ๆ** (ตาราง datagrid) หรือที่ข้อมูล**โตไม่มีที่สิ้นสุด** (บิลขาย) การไล่ทำทุก endpoint จะทำให้สัญญา API พังเป็นวงกว้าง
โดยไม่มีใครได้ประโยชน์ และเพิ่มพื้นที่ให้เกิด bug โดยเปล่าประโยชน์

**หมายเหตุสำหรับหน้าขายสินค้าและหน้าโปรโมชั่น**: `GET /api/v1/products` ถูกใช้โดยหน้า `/sales` และช่องเลือก
สินค้าในหน้า `/promotions` ด้วย ซึ่งไม่ควรมีปุ่มเลขหน้า — แต่ **"ไม่มีปุ่มเลขหน้า" ไม่ได้แปลว่า "ไม่แบ่งหน้า"**
สองหน้านี้ตั้งใจแสดง *ทุกรายการ* จึงต้องไล่อ่านทีละหน้าจนครบ (FR-037) พฤติกรรมการค้นหา/สแกนบาร์โค้ดไม่เปลี่ยน

> **แก้เมื่อ 2026-09-10 — finding F1 จาก `/speckit-analyze`**: เดิมข้อนี้เขียนว่า "ส่ง `pageSize` ที่ใหญ่พอ
> (เช่น 100) และอ่านเฉพาะ `items`" ซึ่งเป็นคำแนะนำที่**ผิด** และถูกนำไปทำจริงจนเกิดบั๊ก — มันตัดสินค้าชิ้นที่
> 101 ทิ้งโดยไม่มีสัญญาณเตือน และหน้า `/promotions` ซึ่งไม่มีช่องค้นหาของตัวเองจะเลือกสินค้านั้นไม่ได้เลย
> วิธีที่ถูกคือ `fetchAllPages()` ใน `web/src/lib/api/client.ts` (T127) ตรวจด้วย
> `web/scripts/verify-fetch-all.cjs` ซึ่งบังคับ `pageSize` เล็กเพื่อให้เกิดหลายรอบจริง
