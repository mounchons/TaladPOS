...
> **หน้าที่ต้องวัด**: `/sales` `/sales/history` `/stock` `/promotions` `/reports` และ `/sales/receipt/{id}`
> (หน้าใบเสร็จต้องใช้ id ของบิลจริง สคริปต์จึงดึงบิลล่าสุดมาก่อนแล้วค่อยวัด)
> สคริปต์วัดต้องใช้ Playwright จึงอยู่นอก `web/` ตาม tasks.md T122 — `web/` ไม่ได้ผูก Playwright ไว้เป็น dependency
...
### 7.7 รายการที่ต้อง "ครบ" ต้องครบจริง และช่องค้นหาช่องเดียวต้องหาได้ทั้งชื่อและบาร์โค้ด

> เพิ่มหลัง `/speckit-analyze` รอบที่ 2 (finding F1) ตรวจ FR-037 และ SC-010

1. **ชั้นวางไม่ตัดทิ้งเงียบ ๆ (FR-037)** — ฝั่ง web เรียก `GET /api/v1/products` ไล่ทีละหน้าจนครบ
   ไม่ใช่ขอหน้าเดียวที่ `pageSize=100` วิธีตรวจที่ไม่ต้องมีสินค้าเกิน 100 ชิ้น: บังคับ `pageSize` เล็ก ๆ
   แล้วดูว่าประกอบครบ

   ```bash
   # คอมไพล์ module จริงแล้วเรียก searchProducts({}, 3) — 21 ชิ้นต้องใช้ 7 รอบ
   cd web && node scripts/verify-fetch-all.cjs
   ```

   สคริปต์คอมไพล์ `src/lib/api/*.ts` ตัวจริงลง temp dir แล้วเรียกจาก node เอง จึงตรวจโค้ดที่แอปใช้จริง
   ไม่ต้องเปิด dev server ต้องการแค่ API ที่ `:8080` (เปลี่ยนได้ด้วย `TALADPOS_API`)

   เกณฑ์ผ่าน: จำนวนที่ประกอบได้ = `totalCount`, ไม่มี id ซ้ำ, และเห็นคำขอมากกว่า 1 หน้า
   (ถ้าเห็นคำขอเดียวแปลว่ากลับไปเป็นเพดานตายตัวอีกแล้ว)

2. **หน้าโปรโมชั่นต้องเลือกสินค้าได้ทุกชิ้น (FR-037)** — เปิด `/promotions` → กด "เพิ่มโปรโมชั่น" →
   นับ `<option>` ใน `<select>` สินค้า ต้องเท่ากับจำนวนสินค้าทั้งหมด (บวกตัวเลือก placeholder)
   หน้านี้ไม่มีช่องค้นหาของตัวเอง ถ้ารายการขาด ผู้จัดการจะเลือกสินค้านั้นไม่ได้เลยและไม่มีอะไรบอก

3. **ช่องค้นหาหน้าขาย: ชื่อ *หรือ* บาร์โค้ด (SC-010)** — ที่ `/sales` พิมพ์ในช่องเดียวกันทีละแบบ

   | พิมพ์ | ต้องได้ |
   |---|---|
   | ว่าง | สินค้าทั้งหมด |
   | ชื่อสินค้า (เช่น `ทุเรียน`) | ทุกชิ้นที่ชื่อมีคำนั้น |
   | บาร์โค้ดเต็ม (เช่น `9990lnj7o`) | ชิ้นนั้นชิ้นเดียว และอยู่บนสุด |
   | คำที่ไม่มีจริง | ไม่มีสินค้า |

   > **กับดักที่เคยพลาดมาแล้ว**: API รับ `search` กับ `barcode` เป็นตัวกรองคนละตัวและ **AND กัน**
   > ส่งคำเดียวกันไปทั้งสองช่องพร้อมกันจะได้ 0 แถวเสมอ (ยกเว้นสินค้าที่ชื่อเท่ากับบาร์โค้ด)
   > ตรวจได้ตรง ๆ: `GET /api/v1/products?search=ทุเรียน&barcode=ทุเรียน` → `totalCount=0`
   > ขณะที่แยกกันได้ 7 และ 0 ตามลำดับ ฝั่ง web จึงต้องยิงแยกแล้วรวมผลเอง
