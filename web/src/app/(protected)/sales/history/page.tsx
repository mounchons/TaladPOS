...
import { dataTablePT, calendarPT, checkboxPT, secondaryButtonPT } from "@/styles/primereact-passthrough";
...
export default function SalesHistoryPage() {
  ...
  return (
    <main className="px-5 py-5">
      ...
      <div className="mb-4 flex flex-wrap items-end gap-4 rounded-control border border-steel-200 bg-white p-4">
        <div>
          <label className="mb-1.5 block text-sm text-ink-700">จากวันที่</label>
          <Calendar value={from} onChange={(e) => setFrom((e.value as Date) ?? null)} dateFormat="dd/mm/yy" pt={calendarPT} showButtonBar />
        </div>
        <div>
          <label className="mb-1.5 block text-sm text-ink-700">ถึงวันที่</label>
          <Calendar value={to} onChange={(e) => setTo((e.value as Date) ?? null)} dateFormat="dd/mm/yy" pt={calendarPT} showButtonBar />
        </div>
        <div className="w-64">
        ...
        <label className="flex items-center gap-2 pb-3 text-sm text-ink-700">
          <Checkbox checked={onlyMine} onChange={(e) => setOnlyMine(e.checked ?? false)} pt={checkboxPT} />
          เฉพาะบิลของฉัน
        </label>
      ...
      >
        <Column
          header="วันที่"
          body={(s: Sale) => <span className="money text-ink-500">{new Date(s.createdAt).toLocaleString("th-TH")}</span>}
        />
        ...
        <Column
          header=""
          body={(s: Sale) => (
            <Button label="ใบเสร็จ" onClick={() => router.push(`/sales/receipt/${s.id}`)} pt={secondaryButtonPT} />
          )}
        />
      </DataTable>
    </main>
